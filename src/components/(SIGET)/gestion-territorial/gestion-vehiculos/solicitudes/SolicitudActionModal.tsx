...
export function SolicitudActionModal({
  ...
  const { data: vehiculosBase = [], isLoading: loadingVehiculos } =
  ...
  const vehiculosParaAprobar = useMemo(() => {
    ...
    const base = vehiculosBase.filter(

      (v): v is VehiculoRow & { id: string } => Boolean(v.id),

    );

    if (!vehiculoPreferido?.id) return base;

    if (base.some((v) => v.id === vehiculoPreferido.id)) return base;
    ...
    return [preferidoComoOpcion, ...base];

  }, [vehiculosBase, vehiculoPreferido]);
              ...
              {loadingVehiculos ? (
                ...
                <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900 dark:bg-amber-950/40">
                  ...
                  <p className="text-sm text-amber-800 dark:text-amber-300">
                    ...
                    No hay vehículos disponibles para asignar en este momento.

...
import { Bell, BellRing, CircleAlert, Wrench } from "lucide";
...
export function MantenimientoNotificaciones({ fallas }: { fallas: FallaRow[] }) {
  ...
  return (
    <GvNotificacionesCampana
      ...
      ariaLabel={`Alertas de mantenimiento${total > 0 ? `, ${total} pendientes` : ""}`}
    >
      <div
        className={cn(
      ...
      >
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">
          {total > 0
            ...
            : "Sin averías activas pendientes"}
        </p>
      </div>
      ...
      {alertas.length === 0 ? (
        <div className="px-4 py-8 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">No hay averías pendientes ni en reparación.</p>
        </div>
      ) : (
